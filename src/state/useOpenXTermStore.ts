...
function sortSessions(sessions: SessionDefinition[]) {
...
function sortMacros(macros: MacroDefinition[]) {
  return [...macros].sort((left, right) => left.name.localeCompare(right.name))
}

function sortSessionFolders(sessionFolders: SessionFolderDefinition[]) {
  return [...sessionFolders].sort((left, right) => left.path.localeCompare(right.path))
}

function buildSessionImportFingerprint(session: SessionDefinition) {
  return [
    session.kind,
...
function summarizeImportedSessions(
...
function joinSessionFolderPath(parentPath: string | null, name: string) {
...
function isFolderPathInSubtree(path: string, subtreeRootPath: string) {
  return path === subtreeRootPath || path.startsWith(`${subtreeRootPath}/`)
}

function replaceFolderPathPrefix(path: string, fromPrefix: string, toPrefix: string) {
  if (path === fromPrefix) {
    return toPrefix
  }
...
function defaultOfflineStatus(): SessionStatusSnapshot {
  return {
    mode: 'offline',
    host: '--',
    user: '--',
...
function appendCpuHistory(history: number[] | undefined, cpuLoad: string) {
...
function mapStatusPayload(payload: SessionStatusPayload): SessionStatusSnapshot {
  return {
    mode: payload.mode,
    host: payload.host,
    user: payload.user,
...
function isLiveTerminalKind(kind: SessionDefinition['kind']) {
...
function nextSessionTabOrdinal(tabs: WorkspaceTab[], sessionId: string) {
